  ...
  <div class="main-legend" :class="{ 'no-tamanos-mapa': !tamanosDeMapa }">
    <div class="header-legend">
      <div class="title-header-legend">
        <input
          type="checkbox"
          class="checkbox-legend-header"
          v-model="prendidosTodos"
          :style="{
            display:
              mostrarBotonAlternaTodos && estiloBotonAlternaTodos === 'checkbox'
                ? ''
                : 'none',
          }"
      ...
      <button
        class="toggle-all"
        v-if="mostrarBotonAlternaTodos && estiloBotonAlternaTodos === 'boton'"
        @click="toogleAll"
      >
        {{ labelToogleAll }}
      </button>
      <button
        class="header-legend-collapsable-boton"
        :class="{ up: !VM_collapsed }"
      ...
      >
        <svg
          ...
          width="17.908"
          height="12.147"
          viewBox="0 0 17.908 12.147"
        >
          <path
            id="Trazado_24243"
            data-name="Trazado 24243"
            d="M8.954,0,0,9.359l2.667,2.788L8.954,5.576l6.287,6.571,2.667-2.788Z"
            transform="translate(17.908 12.147) rotate(180)"
          />
        </svg>
      </button>
    </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      //Indica si alguno, de todas las leyendas esta 
      algunoActivo: true,
      prendidosTodos:true,
      VM_collapsed: false,
    };
  ...
  methods: {
    _draw_legends_from_layers: function () {
      ...
      let interseccion_layers = layers_in_params.filter((value) =>
        layers_in_map.includes(value)
      );
      this.VM_legends = [];
    ...
    checkLabelToggleAll: function () {
      ...
      this.labelToogleAll = estadoTodas.some((item) => item)
        ? "Quitar todos"
        : "Mostrar todos";
...
.header-legend {
  ...
  input[type="checkbox"].checkbox-legend-header {
    ...
    &:indeterminate{
        background-image: url('data:image/svg+xml;charset=utf-8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M400 288h-352c-17.69 0-32-14.32-32-32.01s14.31-31.99 32-31.99h352c17.69 0 32 14.3 32 31.99S417.7 288 400 288z"/></svg>');
    }
  }
