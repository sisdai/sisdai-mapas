        ...
        _createLayerObject: function() {
            let vectorSource =
                this.datos != undefined
                    ? createTopoJSONSourceFromObjectJs(this.datos)
                    : createTopoJSONSourceFromUrl(this.url);
    ...
    watch: {
        datos: function(newDatos) {
            if (
                ...
                let features = new TopoJSON().readFeatures({ ...newDatos });
...
const createTopoJSONSourceFromObjectJs = JsObject => {
    ...
    return new VectorSource({
        features: new TopoJSON().readFeatures({ ...JsObject })
    });
};
